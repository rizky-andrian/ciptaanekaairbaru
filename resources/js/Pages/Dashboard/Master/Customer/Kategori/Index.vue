<template>
    <div class="p-3 rounded-xl bg-white flex flex-col gap-4">
        <!-- Title Pages -->
        <title-pages :backto="'/dashboard/master/customer'">
            Master Bidang Customer
        </title-pages>
        <!-- Endt Title Pages -->
        <!-- Mesasge -->
        <div
                ...
                <div class="mx-2 flex justify-between py-2">
                    <div>
                        <input
                            type="search"
                            name=""
                            placeholder="Search Bidang Customer"
                            id=""
                        ...
                        >
                            <div class="flex flex-col gap-2">
                                <label class="text-gray-700"
                                    >Nama Kategori Bidang</label
                                >
                                <input
                                    class="drop-shadow-sm border py-2 px-3 rounded-md focus:outline-none text-sm"
                                    type="text"
                                    name="Nama Kategori Bidang"
                                    placeholder="Nama Kategori Bidang"
                            ...
                            <div class="flex flex-col gap-2">
                                <label class="text-gray-700"
                                    >Deskripsi Kategori Bidang</label
                                >
                                <textarea
                                    class="drop-shadow-sm border py-2 px-3 rounded-md focus:outline-none text-sm"
                                    name=""
                                    ...
                                    cols="10"
                                    rows="3"
                                    placeholder="Dekskripsi Kategori Bidang"
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            if (!this.updateMode) {
                router.post(
                    "/dashboard/master/customer/create-kategori-bidang",
                    this.form,
                    {
                        preserveScroll: true,
                        onSuccess: () => {
                ...
                router.post(
                    "/dashboard/master/customer/update-kategori-bidang/" +
        ...
        showData(id) {
            this.modalShow = true;
            this.singelData = this.customer.data.filter((data) => data.id == id);
        },
        showModal(id) {
            this.modalCreate = true;
            this.updateMode = true;
            this.singelData = this.customer.data.filter((data) => data.id == id);
            this.form.nama_bidang = this.singelData[0].nama_bidang;
            this.form.deskripsi_bidang =
                this.singelData[0].deskripsi_bidang;
        },
        deleteCheck(id) {
        ...
        deleteMultiple() {
            router.post(
                "/dashboard/master/customer/delete-kategori-bidang",
                this.deleteForm.idCustomer,
                {
                    preserveScroll: true,
                    onSuccess: () => {
